...
// ⚠️ CẢNH BÁO: API Key này cần được di chuyển sang backend
...
export function GeoGebraModal({ isOpen, onOpenChange }: GeoGebraModalProps) {
  ...
  // ✅ FIX 1: Cải thiện logic tải script
  useEffect(() => {
    const scriptSrc = 'https://www.geogebra.org/apps/deployggb.js';

    // Kiểm tra nếu GGBApplet đã có sẵn
  ...
  const destroyGeoGebraApplet = useCallback(() => {
    ...
    try {
      if (ggbAppletRef.current) {
        try {
          if (typeof ggbAppletRef.current.remove === 'function') {
            ggbAppletRef.current.remove();
          }
        } catch (err) {
          console.warn('⚠️ Error removing applet:', err);
        }
      ...
      console.error('❌ Error destroying applet:', err);
    }
  }, []);

  // ✅ FIX 3: Cải thiện logic khởi tạo với timeout dài hơn
  const initializeGeoGebra = useCallback(() => {
    if (isInitializingRef.current || ggbAppletRef.current || !ggbContainerRef.current) {
      console.log('⏭️ Skipping init - already initializing or initialized');
      return;
    }

    if (!isGgbScriptLoaded) {
      console.log('⏳ Waiting for GeoGebra script to load...');
      return;
    }

    if (typeof window.GGBApplet === 'undefined') {
      console.error('❌ GGBApplet not found on window');
      setGeogebraError('GeoGebra chưa được tải. Vui lòng tải lại trang.');
      return;
    }
    ...
    // ✅ Tăng timeout từ 100ms lên 300ms
    initTimeoutRef.current = setTimeout(() => {
      if (!ggbContainerRef.current) {
        console.error('❌ Container lost during init');
        ...
        if (ggbContainerRef.current && ggbAppletRef.current) {
          ...
          if (width > 0 && height > 0) {
            try {
              ...
              console.warn('⚠️ Failed to resize on sidebar change:', err);
            }
          }
        }
      }, 350);
  ...
  useEffect(() => {
    return () => {
      ...
      try {
        ...
        console.warn('⚠️ Error during unmount:', err);
      }
      if (resizeObserverRef.current) resizeObserverRef.current.disconnect();
      if (resizeTimeoutRef.current) clearTimeout(resizeTimeoutRef.current);
      if (initTimeoutRef.current) clearTimeout(initTimeoutRef.current);
  ...
  const handleRetryLoad = () => {
    ...
    if (isOpen && isGgbScriptLoaded) {
      setTimeout(() => initializeGeoGebra(), 500);
    }
  };
